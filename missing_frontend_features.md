...
- **Seleção de Arena**:
  ...
  - **Falta**: Interface para visualizar arenas bloqueadas/desbloqueadas utilizando o `PopupManager`, lógica para selecionar a arena baseada nos troféus do jogador e navegação entre arenas.
- **Seleção de Modos de Jogo**:
  - Não existe lógica para escolher entre diferentes tipos de debate (ex: "Clássico", "Rápido", "Ranqueado").
  - **Falta**: Menu de seleção de modo de jogo antes de iniciar a batalha.
- **Matchmaking Visual**:
  - O botão "Batalha" redireciona imediatamente para `game.html`.
  - **Falta**: Tela de "Procurando Oponente", animação de pareamento, exibição do perfil do oponente antes da partida começar.
- **Deck Builder / Seleção de Filósofos**:
  - O `gameState` possui uma coleção, mas não há "Deck" ou "Time" definido.
  - **Falta**: Interface para o jogador escolher quais 4-8 filósofos levará para o debate (Deck Building).

## 2. Sistema de Economia e Recompensas
- **Gasto de Recursos**:
  - Existem variáveis para `scrolls` (Papiros) e `books` (Tomos/Ouro), mas não há onde gastá-los.
  - **Falta**: Loja (Shop) para comprar cartas, baús ou ouro com gemas, sistema de recompensas ao ganhar uma partida.
- **Coleta de Recompensas (Claim)**:
  - A lógica de abrir baús existe parcialmente (`handlePlayScreenClick`), mas é simplista.
  - **Falta**: Animações visuais de abertura de baú, revelação carta-por-carta (efeito de suspense), e conversão visual de cartas repetidas em "XP" ou "Curingas".
- **Recompensas de Partida**:
  - O `endGame` em `game.js` apenas mostra uma mensagem.
  - **Falta**: Tela de "Vitória/Derrota" rica, mostrando ganho de Troféus, Ouro e Baús ganhos na partida. Atualização do `gameState` com esses ganhos.

## 3. Sistema de Progressão (Meta-Game)
- **Level Up do Jogador**:
  - `gameState` tem `xp` e `level`, mas não há lógica que verifique `if (xp >= xpMax) levelUp()`.
  - **Falta**: Animação de "Level Up", aumento de status da torre/rei, e recompensas por subir de nível.
